-- Migration: hermes_business_lot2_fix_consultation_guards (project smubxqorirlfldatzmym)
-- Fix: consultation blocks must not swallow write phrasings or chantier names
-- that happen to contain a consultation keyword (e.g. a chantier named
-- "Planning Target"). Require a read verb + exclude qualif/planifie.
...
begin
  ...
  -- Chantiers consultation (read verb required; excludes write phrasings).
  ...
  -- Planning consultation (read verb required; excludes write "planifie"/"qualif").
  ...
  -- Devis consultation (read verb required; excludes write phrasings).
